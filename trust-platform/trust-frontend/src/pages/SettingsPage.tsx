      ...
      <main className="main-area">
        ...
        {isOwner && (
          <div className="card" style={{ marginBottom: 14 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 14 }}>
              رتّب أهدافك (1 = أقل أهمية، 5 = أعلى أولوية) — يستخدمها محرك القرار حاليًا للتأثير على حساسية إعادة الطلب وسقف السيولة المسموح به للشراء؛ بقية الأهداف مسجّلة لاستخدامها في محركات قادمة.
            </p>
            {goals === null ? (
              <p style={{ color: 'var(--text-secondary)' }}>جاري التحميل...</p>
            ) : (
              <>
                {goals.map((g) => (
                  <div key={g.type} className="recommendation-row">
                    <span className="rec-title">{goalLabel[g.type]}</span>
                    <select
                      value={g.priority}
